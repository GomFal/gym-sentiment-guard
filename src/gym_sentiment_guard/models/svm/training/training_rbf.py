"""SVM RBF training module.

Provides train_rbf_from_config for reproducible SVM RBF model training from YAML config.
Mirrors the linear training module architecture.
"""
...
def train_rbf_from_config(config_path: str | Path) -> dict:
    """
    Train SVM RBF model from YAML configuration file.
    ...
    use_scaler = clf_cfg.get('use_scaler', True)  # Default True for backward compatibility

    # Build pipeline using shared builder
    model = build_rbf_pipeline(
        ...
        # SVC RBF params
        kernel=clf_cfg.get('kernel', 'rbf'),
        C=clf_cfg.get('C', 1.0),
        gamma=clf_cfg.get('gamma', 'scale'),
        tol=clf_cfg.get('tol', 0.001),
        cache_size=clf_cfg.get('cache_size', 1000),
        max_iter=clf_cfg.get('max_iter', -1),
    ...
    # Train
    log.info(json_log('train.start', component='svm_rbf_training', config=str(config_path)))
    model.fit(x_train, y_train)
    log.info(json_log('train.completed', component='svm_rbf_training'))
    ...
    # Save model
    model_path = out_dir / 'svm_rbf.joblib'
