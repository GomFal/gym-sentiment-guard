...
Creates the main error analysis table with predictions, margins, and coverage.
"""
...
def build_error_table(
    ...
    # Vectorized operations for performance (per python-performance guidelines)
    if issparse(tfidf_matrix):
        # Use CSR internal structure for O(1) nnz computation
        # indptr diff gives nnz per row without creating temporary objects
        nnz_values = np.diff(tfidf_matrix.indptr).astype(np.int32)
        # Vectorized row sum using scipy's optimized implementation
        tfidf_sum_values = np.asarray(tfidf_matrix.sum(axis=1)).ravel()
    else:
        nnz_values = np.count_nonzero(tfidf_matrix, axis=1).astype(np.int32)
